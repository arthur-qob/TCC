...
const RelatoriosPage = () => {
	...
	const [selectedPedidoForMotorista, setSelectedPedidoForMotorista] =
		useState<number | null>(null)
	const [modalConfirmarMotorista, setModalConfirmarMotorista] =
		useState<boolean>(false)
	const [selectedMotoristaTemp, setSelectedMotoristaTemp] = useState<{
		id: number
		nome: string
	} | null>(null)
	...
	const handleSelecionarMotorista = (motoristaId: number, motoristaNome: string) => {
		setSelectedMotoristaTemp({ id: motoristaId, nome: motoristaNome })
		setModalConfirmarMotorista(true)
	}

	const handleConfirmarAtribuicao = () => {
		if (selectedPedidoForMotorista === null || selectedMotoristaTemp === null) return

		// Atualizar o pedido com o motorista selecionado
		setPedidos((prevPedidos) =>
			prevPedidos.map((pedido) =>
				pedido.id === selectedPedidoForMotorista
					? { ...pedido, motoristaId: selectedMotoristaTemp.id }
					: pedido
			)
		)

		// Fechar modais e limpar states
		setModalConfirmarMotorista(false)
		setModalMotoristaIsOpen(false)
		setSelectedPedidoForMotorista(null)
		setSelectedMotoristaTemp(null)
	}
	...
	const columns: Column<PedidoFormatado>[] = [
			...
			render: (value, pedido) =>
				value ? (
					...
					<button
						...
						onClick={(e) => {
							e.stopPropagation()
							setSelectedPedidoForMotorista(Number(pedido.id))
		...
		},
		{
			...
			render: (_value, pedido) => {
				// Se tem motorista e está pendente, mostrar botão de aprovar
				if (
					pedido.motoristaId &&
					...
					return (
						<button
							...
							onClick={(e) => {
								e.stopPropagation()
								handleAprovarPedido(Number(pedido.id))
							}}
							title='Aprovar Pedido'>
							<div className='flex items-center gap-2 text-green-500 hover:text-green-600 transition-colors'>
				...
				// Caso contrário, mostrar botão de visualizar
				return (
					<button
						type='button'
						className='bg-transparent border-none p-0 m-0 outline-none'
	...
	return (
		<Container
			...
			<Modal
				...
				<div className='p-6'>
					...
					<p className='text-sm text-gray-600 mb-6'>
						Selecione um motorista para atribuir ao pedido #
						{selectedPedidoForMotorista}
					</p>

					{/* Lista de Motoristas */}
					<div className='space-y-3 max-h-[400px] overflow-y-auto'>
						{mockUsuarios
							.filter((u) => u.tipo === UserRoles.MOTORISTA)
							.map((motorista) => (
								<button
									key={motorista.id}
									type='button'
									onClick={() =>
										handleSelecionarMotorista(motorista.id, motorista.nome)
									}
									className='w-full text-left p-4 border rounded-lg hover:bg-blue-50 hover:border-blue-500 transition-all duration-300 cursor-pointer'>
			...
			<Modal
				isOpen={modalConfirmarMotorista}
				onClose={() => {
					setModalConfirmarMotorista(false)
					setSelectedMotoristaTemp(null)
				}}>
				<div className='p-6'>
					...
					<p className='text-base mb-6'>
						Deseja realmente atribuir o motorista{' '}
						<strong>{selectedMotoristaTemp?.nome}</strong> ao pedido #
						{selectedPedidoForMotorista}?
					</p>
